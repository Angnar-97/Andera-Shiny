# Módulo: Grafos
# Red de similaridad entre muestras o taxa con phyloseq::plot_net(): los nodos
# se conectan si su distancia es menor que el umbral.
  ...
  tabItem(
    ...
    fluidRow(
      ...
      box(
        title = "Red",
        status = "primary", solidHeader = TRUE, collapsible = TRUE, width = 12,
        shinycssloaders::withSpinner(plotOutput(ns("networkPlot"))),
        br(),
        downloadButton(ns("download_graph"), "Descargar red (.png)")
      )
    )
  )
}
